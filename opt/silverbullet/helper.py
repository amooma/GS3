# -*- coding: utf-8 -*-
# AMOOMA Silver Bullet Helper Library
...
def crc16(packet):
	crc=0x0000
	table = (0, 49345, 49537, 320, 49921, 960, 640, 49729, 50689, 1728,
...
def array_value(array, keys, default = None, cast = False):
	try:
		if (not (isinstance(array, dict) or isinstance(array, list) or isinstance(array, tuple))):
			return default
		...
		for level in range(0, levels):
			array_level = array_level[keys[level]]

		if (cast):
			return cast(array_level)
		else:
			return array_level
...
def set_array_value(array, keys, value, cast = False):
	return False
