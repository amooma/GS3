...
//class Console_Getopt {
	...
	function Console_Getopt_getopt( $args, $short_options, $long_options = null, $cliHelp=null )
	{
		...
		if (isset($args[0]{0}) && $args[0]{0} != '-') array_shift($args);
		reset($args);
		...
		for ($i = 0; $i < count($long_options); $i++) {
			...
			if (substr($long_opt, -1) == '=') {
				if (substr($long_opt, -2) != '==') {
					...
					//var_dump($opt_arg);
					if ( ($opt_arg===null || $opt_arg===false)
					  && !(list(, $opt_arg) = each($args)) )   // fixed to allow empty strings --philipp
			...
			//$opts[] = array('--' . $opt, $opt_arg);
			$realOptDef = explode('=', $long_opt);
			$opts[] = array('--' . $realOptDef[0], $opt_arg);
			return;
		}
...
function handleGetOptError( $err, $shortOpts, $longOpts, $cliHelp=null ) {
	if (Getopt_isError( $err )) {
		echo $err->msg ,"\n";
		getOptUsageOut( $shortOpts, $longOpts, $cliHelp );
		echo "\n";
		die(1);
	}
}

function getOptUsageOut( $shortOpts, $longOpts, $cliHelp=null ) {
	if ($cliHelp)
		echo $cliHelp ,"\n";
	else {
		echo "Options: \n";
...
function getOptsNoMultiples( $short, $long, $cliHelp=null )
{
	$short .= '?';
	...
	if (! Getopt_isError($args)) {
		...
		if (is_array($o[0])) {
			foreach ($o[0] as $oo) {
				$optName = $oo[0];
				if (subStr($optName,0,2)=='--') $optName = subStr($optName,2);
				$opts[$optName] = $oo[1];
			}
		}
	} else {
