...
function gs_pickupgroup_user_add( $pgroup_id, $user )
{
	...
	if ($num > 0)
		return new GsError( 'User already in the group.' );
	
	# add user to the group
	#
	$ok = $db->execute( 'INSERT INTO `pickupgroups_users` (`group_id`, `user_id`) VALUES ('. $pgroup_id .', '. $user_id .')' );
	if (! $ok)
		return new GsError( 'Failed to add user to the group.' );
	
	# add user interface to the pickup group hint
	# ...
