...
include_once( GS_DIR .'inc/log_auth.php' );
...
function gs_host_del( $host,  $force=FALSE )
{
	if (! preg_match( '/^[0-9\.]+$/', $host ))
		return new GsError( 'Host must be a numeric ID or IP.' );
	...
	if ($count_users > 0) 
		 return new GsError( 'Cannot delete host. Delete '.$count_users.' user(s) attached to this host first.' );

	# delete host
	#
	$rs = $db->execute( 'DELETE from `hosts` WHERE `id`=\''. $db->escape($host['id']) .'\'' );
	if (! $rs)
		return new GsError( 'Cannot delete host '.$host['id'] );
