...
function fax_get_jobs_rec() {
	...
	if ((!$conn_id) || (!$login_result)) return 0;
	...
	foreach ($rlist as $rlist_line) {
		$rlist_line_array = explode("|",$rlist_line);
		$jobs_r[]=$rlist_line_array;
	}
	ftp_close($conn_id);
	return $jobs_r;
...
function fax_delete_file($file) {
	...
	$login_result = ftp_login($conn_id, "webmanag", "ablue7");
	if ((!$conn_id) || (!$login_result)) return 0;
	if (!ftp_raw($conn_id, 'admin ablue7')) return 0;
