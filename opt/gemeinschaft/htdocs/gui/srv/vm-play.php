...
if ($fmt === 'mp3') {
	...
	$cmd = $sox.' -q -t al '. qsa($origfile) .' -r 8000 -c 1 -s -2 -t wav - 2>>/dev/null | '.$lame.' --preset fast standard -m m -a -b 32 -B 96 --quiet --ignore-tag-errors --tt '. qsa($id3_title) .' --ta '. qsa($id3_artist) .' --tl '. qsa($id3_album) .' --tc '. qsa($id3_comment) .' --tg 101 - '. qsa($outfile) .' 2>&1 1>>/dev/null';
...
elseif ($fmt === 'wav-pcm') {
	...
	$cmd = $sox.' -q -t al '. qsa($origfile) .' -r 8000 -c 1 -s -2 -t wav '. qsa($outfile) .' 1>>/dev/null';
	$err=0; $out=array();
	@exec( $cmd, $out, $err );
	if ($err != 0) {
		gs_log( GS_LOG_WARNING, 'Failed to convert voicemail file to '.$fmt.'. ('.trim(implode(' - ',$out)).')' );
...
$cmd = 'sox -t al '. qsa($origfile) .' -r 8000 -c 1 -s -2 -t wav '. qsa($intermedfile);
$err=0; $out=array();
@exec( $cmd, $out, $err );
if ($err != 0) {
	gs_log( GS_LOG_WARNING, 'Failed to convert voicemail file.' );
