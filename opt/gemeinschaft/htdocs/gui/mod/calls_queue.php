...
if (@$_REQUEST['action'] === 'del') {
	...
	if ($del_type === $type) {
		...
		$time = @$_REQUEST['time'];
		$DB->execute( 'DELETE FROM `dial_log` WHERE `user_id`='. ((int)@$_SESSION['sudo_user']['info']['id']) .' AND `type`=\''. $DB->escape($type) .'\' AND `timestamp`=\''. $DB->escape($time) .'\' AND `number`=\''. $DB->escape($del_number) .'\'' );
	}
}
...
if (@$rs) {
	...
	while ($r = $rs->fetchRow()) {
		...
		echo "\n";
		echo '<a href="', gs_url($SECTION, $MODULE, null, 'action=del&amp;type='.rawUrlEncode($type) .'&amp;number='.rawUrlEncode($r['number']) .'&amp;time='.rawUrlEncode($r['ts']) .'&amp;page='.$page), '" title="', __('l&ouml;schen'), '" style="margin-left:1.5em;">',
