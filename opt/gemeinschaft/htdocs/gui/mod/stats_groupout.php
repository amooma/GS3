...
for ($day=1; $day<=$num_days; ++$day) {
	...
	$n_calls_out = (int) $CDR_DB->executeGetOne( $sql_query );
...
if ($n_calls_out) {
	...
	$n_calls_out_stat = $n_calls_wait_lower + $n_calls_wait_higher;
	$n_calls_answer = $n_calls_dur_lower + $n_calls_dur_higher;
//	$n_calls_noanswer = $n_calls_out - $n_calls_answer - $n_calls_busy;
	$n_calls_noanswer = $n_calls_out_stat - $n_calls_answer ;	
	$pct_connected = ($n_calls_answer > 0)
		? ($n_calls_answer / $n_calls_out_stat  )
		: 0.0;
	$pct_connected = round($pct_connected*100);

	$totals['n_calls_out'] += $n_calls_out;
	$totals['n_calls_out_stat'] += $n_calls_out_stat;
...
//	echo '<tr class="', ($is_weekend ? 'even':'odd') ,'">', "\n";
//	echo '<td class="r"',$style_wd,'>', $day ,'.</td>', "\n";
	echo '<td class="r"',$style_wd,'>', $n_calls_out ,'</td>', "\n";
	echo '<td class="r"',$style_wd,'>', $n_calls_out_stat ,'</td>', "\n";
...
$pct_connected_month = ($totals['n_calls_answer' ] > 0)
		? ($totals['n_calls_answer' ] / $totals['n_calls_out_stat'   ]   )
		: 0.0;
...
echo '<td class="r" ',$style,'>', $totals['n_calls_out'   ] ,'</td>', "\n";
echo '<td class="r" ',$style,'>', $totals['n_calls_out_stat'   ] ,'</td>', "\n";
