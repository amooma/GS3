...
my $CACHEDIR = '/tmp/hastenix_cache';
...
sub xmlStart
{
	...
	elsif (($startTag eq 'submit') && $isRecord){
...
sub xmlEnd
{
	...
	elsif ($endTag eq 'catch') {
		$isCatch = 0;  # false
	}
	
	elsif ($endTag eq 'record') {
		$isRecord = 0;  # false
	}
}
...
sub sound_creator_thread
{
	...
	foreach my $prompt (@{$prompts}) {
		...
		while (($abort_flag == 0)
		&&     ($soundfile_queue->pending() > $FORERUN)) {
