		...
		if (length($text) > 0)
		{
			...
			unless (-e $wavfile) {
				...
				if ($TTS eq 'cepstral') {
					...
					system($SWIFT . " -p audio/channels=1,audio/sampling-rate=8000" .
								" -o " . $wavfile .
								" -n " . $SWIFT_SPEAKER .
								" -e utf-8" .
...
sub background_player {
	...
	if ($vxmlDataRef->{'redirect'}) {
		...
		my %http_req = ( url => determine_global_url($vxmlDataRef->{'redirect'}, $url),
										 method => 'get' );
		return(\%http_req);
	}
	...
	if (defined($vxmlDataRef->{'record'}->{'name'}) &&
		...
		my %http_req = ( url => determine_global_url($vxmlDataRef->{'record'}->{'url'}, $url),
										 method => 'post',
										 filename => $soundfile,
...
sub handle_url {
	...
	my $sound_creator = threads->new(\&sound_creator_thread,
